...
# read solutions
s <- readRDS("APU/output/APU_solutions_PAs.rds")
...
write_csv(species_targets_spread, "APU/results/APU_representation_targets_by_species_PAs.csv")


# SUMMARIZE AREAS
# read planning units geometries
pu_sf <- read_sf("APU/output/APU_solutions_PAs.shp")
...
# Write table as csv
output_table <- write_csv(solution_table, "APU/results/APU_area_summary_PAs.csv")

# read in the summary targets table
summary_targets <- read_csv("APU/output/summary_targets_PAs.csv")

# join targets and areas table
join_table = inner_join(solution_table, summary_targets, by = 'target')

# write output
output_table <- write_csv(join_table, "APU/results/APU_area_targets_summary_PAs.csv")
