...
#### LOAD FILES SCRIPT ####
# cost
# cost with protected areas data
cost <- read_csv("inputs/cost_pa.csv") %>%
  # set all column headings to lower case
  rename_all(tolower) %>%
  rename(id = hydroid)

# determine which planning units are locked out/in
locked_in <- cost$pa_50 %>% as.logical()
...
p <- problem(cost, species, "cost", rij = rij) %>% 
  ...
  add_locked_in_constraints(which(locked_in))
...
write_csv(s_cost, "APU/output/summary_costs_PAs.csv")
write_csv(s_repr, "APU/output/summary_targets_PAs.csv")
...
write_sf(pu_sf, "APU/output/APU_solutions_PAs.shp")
saveRDS(s, "APU/output/APU_solutions_PAs.rds")
...
#   labs(title = "Selected planning units at different target levels under current climate scenarios with protected areas locked in",
#        fill = NULL) +
#   facet_wrap(~ targets, nrow = 3) +
#   theme_bw() +
#   theme(legend.position = "bottom")
# ggsave("APU/figures/APU_solutions_map_PAs.png", g, width = 15, height = 15)
